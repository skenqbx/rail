...
var SendBuffer = require('./send-buffer');
...
Call.prototype.__request = function(opt_callback) {
  ...
  if (this._buffer) {
    this._buffer.replay(request, function() {
      opt_callback();
...
Call.prototype._write = function(chunk, encoding, callback) {
  if (this._buffer) {
    if (!this._buffer.push(chunk, encoding)) {
      // TODO: bail-out the buffer, max limit reached
      callback(); // ZALGO!
    } else {
      callback(); // ZALGO!
    }
...
Call.prototype.end = function(chunk, encoding, opt_callback) {
  ...
  if (this._buffer || this.__request()) {
    this._end(chunk, encoding, function() {
      if (self._buffer) {
        if (self.request) {
          throw new Error('We should not have a request here');
        }
